...
namespace Captain.Common.Views.Forms
{
    public partial class CASE0006_CAMSForm : Form
    {
        ...
        public CASE0006_CAMSForm(BaseForm baseform, string CAMS_flg, string CAMS_desc, string hierarchy, CASEACTEntity pass_entity, PrivilegeEntity privileges, string MST_site, string MST_intakeWorker, List<FldcntlHieEntity> caFldcEntity, CASESP0Entity sp_header_rec, List<CASEACTEntity> CA_template_list, string sp_start_Date, string spm_site, string sp_End_Date)
        {
            ...
            {
                this.Text = " Service Posting - Edit ";
            ...
            {
                this.Text = " Service Posting - Add " + (CA_Template_List.Count > 0 ? "(Template)" : "");
            ...
            txtMSAmt.Validator = TextBoxValidation.FloatValidator;
        ...
        public CASE0006_CAMSForm(BaseForm baseform, string CAMS_flg, string CAMS_desc, string hierarchy, string year, CASEMSEntity pass_entity, PrivilegeEntity privileges, string MST_site, string MST_intakeWorker, List<FldcntlHieEntity> msFldcEntity, CASESP0Entity sp_header_rec, List<CASEMSEntity> MS_template_list, string sp_start_Date, string spm_site, string sp_End_Date, List<CASESP2Entity> SP2_CAMS_Details)
        {
            ...
            else
            {
                ...
                {
                    pnlEstimateoutcome.Visible = false;
                    this.Size = new System.Drawing.Size(this.Size.Width, this.Size.Height - pnlEstimateoutcome.Height - 40);
                }
            ...
            if (SP2_MS_Details[0].SP2_COUNT_TYPE == "S" || SP2_MS_Details[0].SP2_COUNT_TYPE == "E" || SP2_MS_Details[0].SP2_COUNT_TYPE == "A")
            { txtMSAmt.Visible = true; lblMSAmt.Visible = true; }
            ...
            if (hasSeekDate)
            {
               // pnlEstimateoutcome.Visible = Visible;
                this.Size = new System.Drawing.Size(this.Size.Width, this.Size.Height + pnlEstimateoutcome.Height);
            }
        }
        public bool hasSeekDate=false;
        ...
        private void Fill_Members_DropDown()
        {
            ...
            if (ds.Tables.Count > 0)
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    int rowIndex = 0;
                    string Name = null, TmpSsn = null, Disable_Desc = null, Relation_Desc = null; string dob = string.Empty, CurrAge=string.Empty;
                    foreach (DataRow dr in dt.Rows)
                    {
                        Name = TmpSsn = Relation_Desc = dob=CurrAge = null;
                        ...
                        if (!string.IsNullOrEmpty(dob))
                        {
                            ...
                            CurrAge= CommonFunctions.CalculationYear(Convert.ToDateTime(dob.ToString().Trim()), Convert.ToDateTime(DateTime.Now.Date.ToShortDateString()));
                        }
                        ...
                        //dr["SNP_AGE"].ToString()
                        rowIndex = Members_Grid.Rows.Add(false, Name, dob,CurrAge , Disable_Desc, Relation_Desc,  dr["RecFamSeq"].ToString(), dr["ClientID"].ToString(), dr["AppNo"].ToString().Substring(10, 1), "N", dr["AppStatus"].ToString(), dr["SNP_EXCLUDE"].ToString());
        ...
        private void Fill_Benefiting_From()
        {
            ...
            if (Pass_MS_Entity.Rec_Type == "I")
            {
                if (SP2_MS_Details.Count > 0)
                {
                    CASESP2Entity SelCAlist = SP2_MS_Details.Find(u => u.CamCd.Trim() == Pass_MS_Entity.MS_Code.Trim() && u.Type1 == "MS");
                    if (SelCAlist != null)
                    {
                        if (!string.IsNullOrEmpty(SelCAlist.SP2_OBF))
                            CommonFunctions.SetComboBoxValue(Cmb_MS_Benefit, SelCAlist.SP2_OBF);
                        else
                        {
                            if (!string.IsNullOrEmpty(BaseForm.BaseAgencyControlDetails.BenefitFrom.Trim()))
                                Cmb_MS_Benefit.SelectedIndex = (int.Parse(BaseForm.BaseAgencyControlDetails.BenefitFrom.Trim()) - 1);
                            else
        ...
        private void Fill_Funding()
        {
            ...
            if (Mode == "Edit" && !string.IsNullOrEmpty(SP_Header_Rec.UsedFunds))
            {
                bool Fund_Exists = false; int Pos = 0, Tmp_Loop_Cnt = 0;
                string Funds_Str = SP_Header_Rec.UsedFunds;
                int Tmp_Curr_Fund_Len = 0;

                foreach (SPCommonEntity Entity in FundingList)
                {
                    Fund_Exists = false; Pos = 0;
                    for (int i = 0; Pos < SP_Header_Rec.UsedFunds.Length; i++)
                    {

                        Tmp_Curr_Fund_Len = (Funds_Str.Substring(Pos, Funds_Str.Substring(Pos, (Funds_Str.Length - Pos)).Length)).Length;

                        if (Entity.Code == SP_Header_Rec.UsedFunds.Substring(Pos, (Tmp_Curr_Fund_Len >= 10 ? 10 : Tmp_Curr_Fund_Len)).Trim())
                        {
                            Fund_Exists = true; break;
                        }
                        Pos += 10;
                    ...
                    if (Fund_Exists)
                    {
                        if (Mode == "Edit")
                        {
                            CmbFunding1.Items.Add(new Captain.Common.Utilities.ListItem(Entity.Desc, Entity.Code, Entity.Active, Color.Red));
                            CmbFunding2.Items.Add(new Captain.Common.Utilities.ListItem(Entity.Desc, Entity.Code, Entity.Active, Color.Red));
                            CmbFunding3.Items.Add(new Captain.Common.Utilities.ListItem(Entity.Desc, Entity.Code, Entity.Active, Color.Red));
        ...
        private void Fill_MS_Controls()
        {
            ...
            txtMSAmt.Text = Pass_MS_Entity.MS_Amount.Trim();
        ...
        private void Btn_MS_Save_Click(object sender, EventArgs e)
        {
            if (isValidate() && Validate_MS_Posting_Dtae())
            {
                ...
                if (CAMS_Desc == "Auto Post Outcome")
                {
                    this.DialogResult = DialogResult.OK;
                    this.Close();
                    return;
        ...
        private void Get_Latest_MS_data()
        {
            ...
            //**if (!string.IsNullOrEmpty(txtMSAmt.Text.Trim()))
                Pass_MS_Entity.MS_Amount = txtMSAmt.Text.Trim();
        }
        ...
        private bool isValidate()
        {
                            ...
                            if (SERVStopEntity != null && Act_Date.Checked)
                            {
                                ...
                                else if (!string.IsNullOrEmpty(Sp_Start_Date.Trim()))
                                {
                                    if (string.IsNullOrEmpty(Sp_End_Date.Trim()))
                                    {
                                        ...
                                        {
                                            _errorProvider.SetError(Act_Date, string.Format("' " + lblActivityDate.Text + "' Should not be Prior to the 'Service Plan Master Date'".Replace(Consts.Common.Colon, string.Empty)));
                                            isValid = false;
                                        }
                                        else
                                            _errorProvider.SetError(Act_Date, null);
                                    ...
                                    else
                                    {
                                        ...
                                        {
                                            _errorProvider.SetError(Act_Date, string.Format("' The " + lblActivityDate.Text + "' Should not be Prior to the 'Service Plan Start Date' OR after the 'Actual Completion Date'".Replace(Consts.Common.Colon, string.Empty)));
                                            isValid = false;
                                        }
                                        else
                                            _errorProvider.SetError(Act_Date, null);
            ...
            else
            {
                            ...
                            if (SERVStopEntity != null && MS_Date.Checked)
                            {
                                ...
                                else if (!string.IsNullOrEmpty(Sp_Start_Date.Trim()))
                                {
                                    if (string.IsNullOrEmpty(Sp_End_Date.Trim()))
                                    {
                                        ...
                                        {
                                            _errorProvider.SetError(MS_Date, string.Format("' " + lblMsDate.Text + "' Should not be Prior to the 'Service Plan Master Date'".Replace(Consts.Common.Colon, string.Empty)));
                                            isValid = false;
                                        }
                                        else
                                            _errorProvider.SetError(MS_Date, null);
                                    ...
                                    else
                                    {
                                        ...
                                        {
                                            _errorProvider.SetError(MS_Date, string.Format("' The " + lblMsDate.Text + "' Should not be Prior to the 'Service Plan Start Date' OR after the 'Actual Completion Date'".Replace(Consts.Common.Colon, string.Empty)));
                                            isValid = false;
                                        }
                                        else
                                            _errorProvider.SetError(MS_Date, null);
                ...
                if (BaseForm.BaseAgencyControlDetails.WorkerFUP.ToString().Trim().ToUpper() == "Y")
                {
                    if (lblFUpOnReq.Visible && dtFollowup.Checked == false)
                    {
                        _errorProvider.SetError(dtFollowup, string.Format(Consts.Messages.BlankIsRequired.GetMessage(), lblFUpOn.Text));
                        isValid = false;
                    }
                    else
                        _errorProvider.SetError(dtFollowup, null);

                    if (lblComDtReq.Visible && dtCompleted.Checked == false)
                    {
                        _errorProvider.SetError(dtCompleted, string.Format(Consts.Messages.BlankIsRequired.GetMessage(), lblComDt.Text));
                        isValid = false;
                    }
                    else
                        _errorProvider.SetError(dtCompleted, null);
                ...
                if (txtMSAmt.Visible == true && lblMSAmtReq.Visible == true)
                {
                    if (string.IsNullOrEmpty(txtMSAmt.Text))
                    {
                        _errorProvider.SetError(txtMSAmt, "Amount is Required");
                        isValid = false;
                    }
                    else
                        _errorProvider.SetError(txtMSAmt, null);
                }
                ...
                foreach (FldcntlHieEntity entity in CntlMSEntity)
                {
                    ...
                    switch (entity.FldCode)
                    {
                        ...
                        case Consts.CASE0006.MS_Seek_Date:
                            if (enabled) { dtMSSeek_Date.Visible = lblMsSeekDate.Visible = pnlEstimateoutcome.Visible = hasSeekDate = true; if (required) lblMSSeekDateReq.Visible = true; } else { dtMSSeek_Date.Visible = lblMsSeekDate.Visible = pnlEstimateoutcome.Visible = hasSeekDate= false; lblMSSeekDateReq.Visible = false; }
                            break;
                        ...
                        case Consts.CASE0006.MS_Fund3:
                            ...
                            break;
                        case Consts.CASE0006.MS_Follow_Date:
                            if (BaseForm.BaseAgencyControlDetails.WorkerFUP.ToString().Trim().ToUpper() == "Y")
                            {
                                if (enabled) { dtFollowup.Visible = lblFUpOn.Visible = true; if (required) lblFUpOnReq.Visible = true; } else { dtFollowup.Visible = lblFUpOn.Visible = false; lblFUpOnReq.Visible = false; }
                                
                            }
                            break;

                        case Consts.CASE0006.MS_Comp_Date:
                            if (BaseForm.BaseAgencyControlDetails.WorkerFUP.ToString().Trim().ToUpper() == "Y")
                            {
                                if (enabled) { dtCompleted.Visible = lblComDt.Visible = true; if (required) lblComDtReq.Visible = true; } else { dtCompleted.Visible = lblComDt.Visible = false; lblComDtReq.Visible = false; }
                                
                            }
                            break;
                        case Consts.CASE0006.Cost:
                            if (enabled) { txtMSAmt.Visible = lblMSAmt.Visible = true; if (required) lblMSAmtReq.Visible = true; } else { txtMSAmt.Visible = lblMSAmt.Visible = false; lblMSAmtReq.Visible = false; }
                            break;
        ...
        private void On_PaymentVoucher()
        {
            ...
            iTextSharp.text.Font TimesUnderline = new iTextSharp.text.Font(bf_times, 10, 4);
